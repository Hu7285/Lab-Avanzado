...
int main(int argc,char *argv[]) {
  ...
  /*-----Se crea un histograma y un par de variables-----*/

  TH1D *mHisto = new TH1D("histograma", "Histograma de tiempo - Oct", 22,490,20000);
  int mcount = 0;       //variable para llevar conteo de eventos relevantes
  int inter = 500;      //intervalo de tiempo entre las placas
  ...
  for(int i=0; i<n_events; i++)
  {
    ...
    /*----Verificacion de eventos relevantes-----*/

    //Se verifica que existe una coincidendia entre las palcas y se asegura que exista un intervalo de tiempo entre las placas

    if(coincidence)
    {
      if(RE0->size() > 0 && RE1->size()>0)
      {
        if((fabs(RE0->at(0)-RE1->at(0)))>inter)
        {
          mHisto->Fill(fabs(RE0->at(0)-RE1->at(0)));
          mcount++;         //Se lleva un conteo de eventos relevantes
        }
      }
    }
  ...
  /*-----Creacion de archivos .root y la grafica*/

  std::cout<< "Eventos de decaimiento de muon: " << mcount << std::endl;

  DataFile = new TFile( "histograma-Oct.root","RECREATE");      //Se crea un archivo para guardar los datos
    mHisto->Write();
    DataFile->Close();
  TCanvas *ShowGraph = new TCanvas("mHisto", "RP aqcuire tools", 600, 400);   //Se crea un canvas para generar la grafica
    mHisto->Draw();
    ShowGraph->Update();
    ShowGraph->SaveAs( "histograma-Oct.png" ,"png");
    DataFile->Close();
